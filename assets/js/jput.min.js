    ...
    try {
      ...
      function getOperator(e) {
        ...
        for (i = 0; i < operators.length; i++)
          if (e.search(operators[i]) != -1) return operators[i];
      }
      function process(options) {
        ...
        $.each(arr, function (i, j) {
          ...
          if (el1_jrepeat.length > 0)
            el1_jrepeat.each(function (e, t) {
              $(this).remove();
            });
          ...
          if (matches != null)
            $.each(matches, function (index, value) {
              ...
              if (c.length == 4) {
                ...
                for (ci = 0; ci < 2; ci++)
                  if (c[ci].substr(-1) + c[ci].charAt(0) != "''")
                    c[ci] = "'" + escape(j[c[ci]]) + "'";
                for (ci = 2; ci < 4; ci++)
                  if (c[ci].substr(-1) + c[ci].charAt(0) != "''")
                    c[ci] = j[c[ci]];
                ...
                if (eval(c[0] + condition + c[1]))
                  html = html.replace(value, unescape(c[2]));
                else html = html.replace(value, unescape(c[3]));
              } else {
                t = t.split("||");
                newval = json_value(t[0], j);
                if ((newval == "" || newval == undefined) && t.length != 1)
                  html = html.replace(value, t[1]);
      ...
      var el =
        options.name == ""
          ? $("[jput]").clone()
          : $('[jput="' + options.name + '"]').clone();
      if (el.length == 0)
        throw "Cant find the jPut template (" + options.name + ")!";
      var regExp = /\{{([^}}]+)\}}/g;
